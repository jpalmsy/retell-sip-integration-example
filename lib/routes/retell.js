...
let phoneQueueMap = {};
try {
  phoneQueueMap = JSON.parse(process.env.PHONE_QUEUE_JSON || '{}');
} catch (e) {
  console.error('PHONE_QUEUE_JSON is not valid JSON – defaulting to empty map.', e);
}
...
const onRefer = (session, evt) => {
  ...
  logger.info({ refer_details }, 'received REFER');

  /* extract E.164 user part */
  const number = (refer_details.refer_to_user || '')
    .replace(/^sip:/i, '')
    .split('@')[0]
    .replace(/[^\d+]/g, '');

  /* look up matching queue URI */
  const mapped = phoneQueueMap[number];
  const referToUri = mapped || refer_details.refer_to_user;

  logger.info(
    mapped
      ? { number, referToUri, note: 'mapped to queue URI' }
      : { number, note: 'no mapping – forwarding original REFER' }
  );

  session.sip_refer({ referTo: referToUri, referredBy: evt.to }).reply();
};
